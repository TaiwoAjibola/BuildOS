...
import { GitCompare, CheckCircle, XCircle, Clock, ChevronRight, Search, DollarSign } from "lucide-react";
import { useState } from "react";
import { projects, changeRequests, fmtDate, fmtCurrency } from "./mockData";
...
export function ChangeRequestsOverviewPage() {
  ...
  const underReview = changeRequests.filter(cr => cr.status === "Under Review");
  const approved = changeRequests.filter(cr => cr.status === "Approved");
  const totalCostImpact = changeRequests.reduce((s, cr) => s + cr.costImpact, 0);

  const stats = [
    { icon: GitCompare, label: "Total CRs", value: changeRequests.length },
    { icon: Clock, label: "Under Review", value: underReview.length, color: "#F4A623" },
    { icon: CheckCircle, label: "Approved", value: approved.length, color: "#27AE60" },
    { icon: DollarSign, label: "Total Cost Impact", value: fmtCurrency(totalCostImpact), color: "#E8973A" },
  ];
  ...
  return (
    <div style={{ backgroundColor: "#F7F8FA" }} className="min-h-screen p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: "#1A202C" }}>Change Requests Overview</h1>
        <p className="text-sm mt-1" style={{ color: "#718096" }}>Change requests across all projects</p>
      </div>
      ...
      <div className="bg-white rounded-lg p-4" style={{ border: "1px solid #E2E8F0" }}>
        <div className="relative mb-4">
          ...
          <input
            type="text" placeholder="Search change requests..."
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ backgroundColor: "#F7F8FA", borderBottom: "1px solid #E2E8F0" }}>
                <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>CR ID</th>
                ...
                <th className="text-right px-4 py-3 font-medium" style={{ color: "#718096" }}>Cost Impact</th>
            ...
            <tbody>
              {filtered.map((cr, i) => {
                ...
                const st = STATUS_STYLES[cr.status] ?? { bg: "#F1F5F9", text: "#475569" };
                return (
                  <tr
                    key={cr.id}
                    className="cursor-pointer hover:opacity-80 transition-opacity"
                    style={{ borderBottom: i < filtered.length - 1 ? "1px solid #E2E8F0" : "none" }}
                    onClick={() => navigate(`/apps/construction/projects/${cr.projectId}/change-requests`)}
                  >
                    <td className="px-4 py-3 font-medium" style={{ color: "#1A202C" }}>{cr.crNumber}</td>
                    <td className="px-4 py-3" style={{ color: "#718096" }}>{project?.name ?? cr.projectId}</td>
                    <td className="px-4 py-3">
                      <div className="flex gap-1">
                        {cr.changeTypes.map(t => {
                          const tc = TYPE_STYLES[t] ?? { bg: "#F1F5F9", text: "#475569" };
                          return (
                            <span key={t} className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: tc.bg, color: tc.text }}>
                              {t}
                            </span>
                          );
                        })}
                    ...
                    <td className="px-4 py-3 max-w-[240px] truncate" style={{ color: "#1A202C" }}>{cr.description}</td>
                    <td className="px-4 py-3">
                      <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: st.bg, color: st.text }}>
                        {cr.status}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right font-medium" style={{ color: "#E74C3C" }}>{fmtCurrency(cr.costImpact)}</td>
                    <td className="px-4 py-3" style={{ color: "#718096" }}>{fmtDate(cr.dateRaised)}</td>
                    <td className="px-4 py-3"><ChevronRight className="w-4 h-4" style={{ color: "#718096" }} /></td>
                  </tr>
                );
              })}
              {filtered.length === 0 && (
                <tr><td colSpan={8} className="text-center py-8 text-sm" style={{ color: "#718096" }}>No change requests found</td></tr>
              )}
            </tbody>
          </table>
        </div>
