...
export function DelaysOverviewPage() {
  ...
  const stats = [
    ...
    { icon: AlertTriangle, label: "Open", value: open.length, color: "#E74C3C" },
    { icon: Clock, label: "Recovery Underway", value: recovery.length, color: "#F4A623" },
    { icon: CheckCircle, label: "Resolved", value: resolved.length, color: "#27AE60" },
  ];

  return (
    <div style={{ backgroundColor: "#F7F8FA" }} className="min-h-screen p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: "#1A202C" }}>Delays Overview</h1>
        <p className="text-sm mt-1" style={{ color: "#718096" }}>Delays across all projects</p>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ backgroundColor: "#F7F8FA", borderBottom: "1px solid #E2E8F0" }}>
                <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Task</th>
                <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Project</th>
                <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Stage</th>
                <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Planned End</th>
                <th className="text-center px-4 py-3 font-medium" style={{ color: "#718096" }}>Days Delayed</th>
                <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Root Cause</th>
              ...
              {delays.map((delay, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3" style={{ color: "#718096" }}>{project?.name ?? delay.projectId}</td>
                    <td className="px-4 py-3" style={{ color: "#718096" }}>{delay.stagePhase}</td>
                    <td className="px-4 py-3" style={{ color: "#718096" }}>{fmtDate(delay.plannedEndDate)}</td>
                    <td className="text-center px-4 py-3 font-medium" style={{ color: "#E74C3C" }}>{delay.daysDelayed}</td>
                    <td className="px-4 py-3 max-w-[200px] truncate" style={{ color: "#718096" }}>{delay.rootCause}</td>
