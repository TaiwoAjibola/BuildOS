...
const INCIDENTS_MOCK = [
  { id: "INC-001", type: "Near Miss", date: "2026-05-15", projectId: "PRJ-001", description: "Worker slipped on wet surface near foundation area", severity: "Low" },
  { id: "INC-002", type: "First Aid", date: "2026-05-10", projectId: "PRJ-002", description: "Minor cut from rebar end cap missing", severity: "Low" },
];
...
export function HSEOverviewPage() {
  ...
  const stats = [
    ...
    { icon: AlertTriangle, label: "Expiring Soon", value: expiringSoon.length, color: "#F4A623" },
  ...
  return (
    <div style={{ backgroundColor: "#F7F8FA" }} className="min-h-screen p-6 space-y-6">
      <div>
        <h1 className="text-2xl font-bold" style={{ color: "#1A202C" }}>HSE Overview</h1>
        <p className="text-sm mt-1" style={{ color: "#718096" }}>HSE competency and incidents across all projects</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr style={{ backgroundColor: "#F7F8FA", borderBottom: "1px solid #E2E8F0" }}>
                  <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Staff</th>
                  <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Competency</th>
                  <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Project</th>
                  <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Date Obtained</th>
                  <th className="text-left px-4 py-3 font-medium" style={{ color: "#718096" }}>Expiry</th>
                ...
                {hseMatrix.map((hse, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3" style={{ color: "#718096" }}>{hse.competency}</td>
                      <td className="px-4 py-3" style={{ color: "#718096" }}>{project?.name ?? hse.projectId}</td>
                      <td className="px-4 py-3" style={{ color: "#718096" }}>{fmtDate(hse.dateObtained)}</td>
                      <td className="px-4 py-3" style={{ color: "#718096" }}>{fmtDate(hse.expiryDate)}</td>
        ...
        <div className="bg-white rounded-lg p-5" style={{ border: "1px solid #E2E8F0" }}>
          <h2 className="text-base font-semibold mb-4" style={{ color: "#1A202C" }}>Recent Incidents</h2>
          <div className="space-y-3">
            {INCIDENTS_MOCK.map(inc => {
              const project = projects.find(p => p.id === inc.projectId);
              const sevColor = inc.severity === "Low" ? "#F4A623" : inc.severity === "Medium" ? "#E8973A" : "#E74C3C";
              return (
                <div key={inc.id} className="p-3 rounded-lg" style={{ backgroundColor: "#F7F8FA", border: "1px solid #E2E8F0" }}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-medium px-2 py-0.5 rounded-full" style={{ backgroundColor: `${sevColor}20`, color: sevColor }}>
                      {inc.type}
                    </span>
                    <span className="text-xs" style={{ color: "#718096" }}>{fmtDate(inc.date)}</span>
                  </div>
                  <p className="text-sm" style={{ color: "#1A202C" }}>{inc.description}</p>
                  <p className="text-xs mt-1" style={{ color: "#718096" }}>{project?.name}</p>
                </div>
              );
            })}
            {INCIDENTS_MOCK.length === 0 && (
              <p className="text-sm" style={{ color: "#718096" }}>No incidents recorded</p>
            )}
          </div>
        </div>
      </div>
