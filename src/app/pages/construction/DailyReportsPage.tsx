...
const weatherIcon = (w: string) => {
  switch (w) {
    ...
    case "Rainy": return <CloudRain className="w-4 h-4 text-blue-600" />;
    default: return <Sun className="w-4 h-4 text-gray-300" />;
  }
};
...
export function DailyReportsPage() {
  ...
  const totalMandays = filtered.reduce((s, r) => s + r.manpower.reduce((m, p) => m + p.mandays, 0), 0);
    ...
    <div className="space-y-6" style={{ backgroundColor: "#F7F8FA" }}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{project?.name || "Project"} · {allReports.length} reports</p>
        </div>
        <button
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-lg border border-[#E2E8F0] p-4">
          <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Total Man-days</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">{totalMandays}</p>
        </div>
        <div className="bg-white rounded-lg border border-[#E2E8F0] p-4">
          <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">Equipment Entries</p>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-[#E2E8F0] bg-gray-50/50">
              ...
              <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 py-3">Weather</th>
              <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 py-3">Submitted By</th>
              <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 py-3">Status</th>
              <th className="text-center text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 py-3">Manpower</th>
              <th className="text-center text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 py-3">Equipment</th>
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-4 py-16 text-center">
                  ...
                  <p className="text-sm text-gray-500 mb-2">No daily reports found</p>
                  <button
                    onClick={() => navigate(`${basePath}/new`)}
              ...
              filtered.map(r => (
                ...
                >
                  <td className="px-4 py-3 text-sm font-medium text-gray-900">{fmtDate(r.reportDate)}</td>
                  <td className="px-4 py-3">{weatherIcon(r.weather)}</td>
                  <td className="px-4 py-3 text-sm text-gray-700">{r.submittedBy}</td>
                  ...
                  <td className="px-4 py-3 text-center text-sm text-gray-700">{r.equipment.length}</td>
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button
                        onClick={e => { e.stopPropagation(); navigate(`${basePath}/${r.id}`); }}
                      ...
                      <button
                        onClick={e => { e.stopPropagation(); navigate(`${basePath}/${r.id}/edit`); }}
                        className="p-1.5 rounded-md hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors"
                        title="Edit"
                      >
