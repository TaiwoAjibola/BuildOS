...
- Landing page now at `/` (was LoginPage) — marketing site: hero, live product preview, 7-module showcase (real launcher colors/metrics), workflow, financial-integrity ledger mock, roles, CTA + footer; links to `/auth/login` and `/auth/signup`. Authored `src/app/pages/LandingPage.tsx` (~470 lines) using the ui-ux-pro-max "Enterprise Gateway" pattern; DM Sans font link added to `index.html`.
  - CTA section fix (a17f8ba follow-up, uncommitted): gradient was inside a `relative` section on `-z-10` divs so it rendered BEHIND the white page (`-z-10` + non-stacking parent means painted behind root background) → white text on white. Now the section owns `bg-slate-900` + gradient + dot layers as stacked absolute siblings, content in a `relative` wrapper. Also added blueprint grid + live-badge in hero, step chips + arrows in workflow, 4-col footer, `text-balance` headings, `scroll-behavior:smooth` in index.html.
